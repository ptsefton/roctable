...
Per table, keyed by `@type`: `ignore_props`, `expand_props`, `load_text`.
Everything not ignored is included implicitly. This is enough for a single
known crate; inspection requires an explicit inventory.

### 5.2 Required additions

- Explicit per-table property list, generated by inspection, not just an
  ignore-list — a user MUST be able to see every candidate property, not
  only reason about what's missing.
- Per-table: output name, and the rule for which entity types map to it
  when an entity carries more than one `@type`.
- Per-property: include flag, rename/label, expand flag, and an explicit
  policy for repeated values (current behaviour — silently drop properties
  with more than five values — MUST become a configurable choice, not a
  fixed limit).
- Whether the root dataset entity is emitted as its own table.
