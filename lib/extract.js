...
// @id too, when there was one, for use in join's `_joined_ID`.
...
// `_joined_ID` and one `_joined_${header}` per CSV column (SPEC.md §5).
// `columnsConfig` is the join property's own nested `columns` map (mirroring
// a property's `properties` map for `expand`, discovered by
// inspect.js's discoverJoinColumns): a column is dropped only if explicitly
// `include: false`, and a column marked `expand: true` has its value treated
// as an @id — dereferenced against `crate` and flattened via `expandInto`
// instead of kept as a plain string. A column this map hasn't seen yet
// (absent from it) still comes through as a plain value, same as an
// unconfigured `expand` sub-property.
function joinRecords(rows, entityId, outputProp, fileId, records, table, crate, columnsConfig) {
  const joined = [];
  records.forEach((record, index) => {
    const joinedId = `${entityId}::${outputProp}::${fileId ?? ""}::${index}`;
    for (const base of rows) {
      const row = { ...base, _joined_ID: [{ name: joinedId, id: null }] };
      bumpKey(table, "_joined_ID", 1);
      for (const [header, value] of Object.entries(record)) {
        if (columnsConfig?.[header]?.include === false) continue;
        const colName = `_joined_${header}`;
        if (columnsConfig?.[header]?.expand) {
          const target = value ? crate.getEntity(value) : null;
          if (target) {
            expandInto(row, table, colName, target, columnsConfig[header].properties);
            continue;
          }
          // No target entity found for expansion: fall back to a plain value below.
        }
...
export async function extractTables(crate, config, { crateDir, fileReader } = {}) {
  ...
  for (const entity of crate.entities()) {
    ...
    for (const entityType of entityTypes) {
      ...
      for (const prop of Object.keys(entity)) {
        ...
        if (propConfig.load_text) {
          ...
          if (propConfig.join === "csv") {
            ...
            joins.push({ outputProp, fileId, records, columnsConfig: propConfig.columns });
          } else {
      ...
      let rows = [row];
      for (const { outputProp, fileId, records, columnsConfig } of joins) {
        rows = joinRecords(rows, entity["@id"], outputProp, fileId, records, table, crate, columnsConfig);
      }
