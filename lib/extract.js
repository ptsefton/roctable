...
// Resolve a `load_text` property's value to a file on disk (relative to the
// crate directory) and read its contents. Returns the referenced entity's
// @id too, when there was one, for use in join's `_concat_ID`.
function loadText(prop, entity, crateDir) {
  ...
  if (fullPath && fs.existsSync(fullPath)) {
    return { text: fs.readFileSync(fullPath, "utf8"), fileId };
  }
  console.warn(
    ...
    for (const entityType of entityTypes) {
      ...
      for (const prop of Object.keys(entity)) {
        ...
        if (propConfig.load_text) {
          const { text, fileId } = loadText(prop, entity, crateDir);
